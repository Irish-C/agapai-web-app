...
async def list_locations(
    ...
    _: None = Depends(require_feature("view_locations"))
):
...
async def create_location(
    request: Request,
    user_id: str = Depends(get_current_user_id),
    _: None = Depends(require_feature("add_location"))
):
...
async def update_location(
    ...
    request: Request,
    user_id: str = Depends(get_current_user_id),
    _: None = Depends(require_feature("edit_location"))
):
    data = await get_sanitized_json(request)
...
async def delete_location(
    location_id: int,
    user_id: str = Depends(get_current_user_id),
    _: None = Depends(require_feature("delete_location"))
):
